...
document.getElementById('sell-form').addEventListener('submit', (e) => {
    ...
    const msg = "✅ Объявление опубликовано! Книга появится в разделе «Купить».";
...
// ========== ЖАНРЫ ДЛЯ ЧТЕНИЯ ==========
document.querySelectorAll('#read-genres .genre-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('#read-genres .genre-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        currentReadGenre = btn.dataset.genre;
        renderReadBooks();
    });
});

// ========== ЖАНРЫ ДЛЯ ПОКУПКИ (ВЫПАДАЮЩЕЕ МЕНЮ) ==========
// Обработка кнопки "Все"
document.querySelectorAll('#buy-genres .genre-btn[data-buy-genre="all"]').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('#buy-genres .genre-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        currentBuyGenre = 'all';
        renderBuyBooks();
    });
...
// Обработка пунктов выпадающего меню
document.querySelectorAll('#dropdownContent button').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.getElementById('dropdownBtn').innerHTML = (genreNames[genre] || '📖 Жанры') + ' ▼';
